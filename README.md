...
HIVYA is a full-stack e-commerce platform covering the complete shopping flow — product discovery, cart, authentication, checkout with real (test-mode) payments, order tracking, and an admin panel for managing the store.
...
```
HIVYA never stores raw card data — Stripe carries that entire scope.
...
### 1. Clone
```bash
git clone https://github.com/<your-username>/hivya.git
cd hivya
```
...
## 📚 What I'm Learning Through HIVYA
...
HIVYA is currently a personal learning and portfolio project. Suggestions and discussion are welcome via Issues.
